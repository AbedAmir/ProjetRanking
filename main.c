...
int main()
{
    ...
    algoAddAndSuppSommetAvecLiensProbaUnSurNblienDuSommet(281000,0,0);
...
void algoAddAndSuppSommetAvecLiensProbaUnSurNblienDuSommet(int nombreSommetSupp, int nombreSommetAjoute, int nombreLiensAjoute)
{
    ...
    while(i<R)
    {
        ...
        if(vecteur_n1_modif[temp] != -1) // condition pour verifier si le sommet n'a pas déja etait supprimé
        {
            //printf("Sommet supp = %d\n",temp);
            sommeProbaSommetDisparu = sommeProbaSommetDisparu + vecteur_n1_modif[temp];
            vecteur_n1_modif[temp] = -1;
            ...
            {
                printf("vecteurFT[%d] = %d\n",t,vecteur_FT_Modif[t]);
            }*/
            //printf("\n");
        }
    }
    ...
    // refresh vecteurFT pour le cas des sommets supp
    /*for(i=0;i<sizeApresModif;i++)
    {
        if(vecteur_FT_Modif[i] == 0)
        {
            cpt=0;
            for(j=0;j<nblienTotalApresModif;j++)
            {
                if(matrice_H_Modif[1][j] == 1)
                {
                    cpt++;
                }
            }
